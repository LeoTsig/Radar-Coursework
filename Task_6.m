...
theta_target = transpose([40, 70, 120]);
R_target = transpose([2000, 3000, 2500]);
RCS_target = transpose([1, 5, 4.5]);
complexity_target = transpose([0, 1, 2]);
...
theta_steer_known = 70;  % SOS: Change the steering angle (only 40 or 70 as per figure 2) to get each plot for each target.
...
xline(PRI,'black');
xlim([0, 11200])
%print(f1, '-vector', ['Point Z - ', num2str(theta_steer_known), 'deg - Task 6'], '-dpng')
...
grid on
%print(f2, '-vector', ['Angle_sweep - 40 and 70 deg - Task 6'], '-dpng')
...
grid on
xlim([0, 11200])
%print(f3, '-vector', ['MF output - ', num2str(theta_steer_known), 'deg - Task 6'], '-dpng')
...
xlim([0, 1400])
ylim([0, 3e-5]);
%print(f4, '-vector', ['Non coherent output - ', num2str(theta_steer_known), 'deg - Task 6'], '-dpng')
