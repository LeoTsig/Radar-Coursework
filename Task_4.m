...
theta_target = transpose([40]); %tranpose are not necessaqry here, they 
% are used to enmphasize that the backscatter function takes collumn vectors for target parameters
R_target = transpose([2000]);
RCS_target = transpose([1]);
complexity_target = transpose([0]);

% Directed Lobe Gains
GTx_direct = 45;
GRx_direct = 45;
%% Assuming target is known (we take for granted that theta_steer must be 40 degrees): 
% Simualtion with known target parameters
theta_steer_known = 40;
...
grid on
xlim([0, 11200])
%print(f1, '-vector', ['Point Z - ', num2str(theta_steer_known), 'deg'], '-dpng')
...
grid on
%print(f2, '-vector', ['Angle_sweep - ', num2str(theta_steer_known), 'deg'], '-dpng')

%% Parameter estimation (azimuth target angle):
theta_target_est = theta_covered(theta_peak == max(theta_peak));
disp(['The target is at ', num2str(theta_target_est), ' degrees!'])
...
grid on
xlim([0, 11200])
%print(f3, '-vector', ['MF output - ', num2str(theta_steer_known), 'deg'], '-dpng')
...
xlim([0, 1400])
ylim([0, 3e-5]);
%print(f4, '-vector', ['Non coherent output - ', num2str(theta_steer_known), 'deg'], '-dpng')
